...
const store = new Vuex.Store({
  ...
  actions: {
    LOAD_PROJECT_LIST: function ({ commit }) {
      axios.get('http://localhost:3000/secured/projects').then((response) => {
    ...
    ADD_NEW_PROJECT: function ({ commit }) {
        axios.post('http://localhost:3000/secured/projects').then((response) => {
    ...
    TOGGLE_COMPLETED: function ({ commit, state }, { item }) {
        axios.put('http://localhost:3000/secured/projects' + item.id, item).then((response) => {
